...
# Configure a device with tags for categorization
resource "wallix-bastion_device" "web_server" {
...
Tags allow you to categorize and filter devices using key-value pairs. Common use cases include:
...
Each tag consists of a **key** and a **value**, both required. You can add multiple tags to a device by repeating the `tags` block.
